    ...
    testplanColumnDefs = [
        {
            ...
            cellRendererParams: {
                ...
                innerRenderer: function (params) {
                    ...
                    } else if (params && params.data && params.data.type !== 'testplan' && params.data.type !== 'test' && params.data.type !== 'potential_test') {
...
function getTypeFilter(data) {
    switch(data.type){
        case 'toggle' : {
            return 'tsca_filter=name,equals,' + data.testplan.split('/').pop();
        }
        case 'cross':
        case 'coverpoint':
            {
            var href = ''
            var startIndex = Math.max(0 ,data.testplan.lastIndexOf('\\'));
            var lastIndex = data.testplan.lastIndexOf('/');
            var instance = data.testplan.substr(startIndex + 1,  lastIndex - startIndex - 1);
            var cross = data.testplan.substr(lastIndex + 1 , data.testplan.length);
            if  (instance){
